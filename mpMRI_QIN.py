...
#%% #%% Brain extraction
    ...
    #Read/edit brain mask generated from FSL BET
...
#%% Generate DPBN maps by using the following formula D_(p,i)= 〖D_min+(〖D_max-D_min)]×〖TP〗_i] 
...
#%%Generate inverse dose prescription as Dinv = Dmax - Dpresc, where Dmax = 80 Gy
    ...
    #Remove CSF from Inv DP map
    Inv_ADC_rBV_DPBN = generate_mask(Inv_ADC_rBV_DPBN, CSF)
    Inv_ADC_rBF_DPBN = generate_mask(Inv_ADC_rBF_DPBN, CSF)

    #Remove voxel values outside brain due to removal of CSF and obtain definitive probability map
    Inv_ADC_rBV_DPBN = generate_mask(Inv_ADC_rBV_DPBN, DSC_mask_inT1CE)
    Inv_ADC_rBF_DPBN = generate_mask(Inv_ADC_rBF_DPBN, DSC_mask_inT1CE)
...
#%%Generate masked images
    ...
    ADC_masked = generate_mask(ADC_bet, Margin_volume)
    sitk.WriteImage(ADC_masked, bet_imgs+'/ADC_masked.nii')

    rBF_masked = generate_mask(rBF_rel, Margin_volume)
    sitk.WriteImage(rBF_masked, rel_imgs+'/rBF_masked.nii')
    
    rBV_masked = generate_mask(rBV_rel, Margin_volume)
    sitk.WriteImage(rBV_masked, rel_imgs+'/rBV_masked.nii')
    
    ADC_rBV_TP_masked = generate_mask(ADC_rBV_def, Margin_volume)
    sitk.WriteImage(ADC_rBV_TP_masked, prob_imgs+'/ADC_rBV_TP_masked.nii')

    ADC_rBF_TP_masked = generate_mask(ADC_rBF_def, Margin_volume)
    sitk.WriteImage(ADC_rBF_TP_masked, prob_imgs+'/ADC_rBF_TP_masked.nii')
    
    ADC_rBV_DP_masked = generate_mask(ADC_rBV_DPBN, Margin_volume)
    sitk.WriteImage(ADC_rBV_DP_masked, dose_imgs+'/ADC_rBV_DP_masked.nii')

    ADC_rBF_DP_masked = generate_mask(ADC_rBF_DPBN, Margin_volume)
    sitk.WriteImage(ADC_rBF_DP_masked, dose_imgs+'/ADC_rBF_DP_masked.nii')
